 ...
 */
public class PreviewDocAdapter extends BaseAdapter {
    ...
    public PreviewDocAdapter(Context context, List<AirInfo.DataBean.AccaServiceBean> accaServiceBeen, List<AirInfo.DataBean.AccaServiceBean> list1) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        item_name = list1.get(position).getItem_name();
        holder.tv_message.setText(list1.get(position).getItem_name());
        holder.tv_amount.setText(list1.get(position).getItem_value());

        start_time = list1.get(position).getStart_time();
        end_time = list1.get(position).getEnd_time();
        ...
        if ("Y".equals(flag)) {
            ...
            String qianxu1 = DateToString.StringToDate(start_time, formatStr, dateFromatStr);
            LogUtil.d("---starttime",start_time);
            String substring1 = qianxu1.substring(5, qianxu1.length());
            holder.create_time.setText(substring1);

            //转换为2016-05-03 02:33:00 的格式
            String qianxu2 = DateToString.StringToDate(end_time, formatStr, dateFromatStr);
